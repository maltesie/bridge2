...
def check_angle(atoms_in_distance, heavy2hydrogen, local_coordinates, hydrogen_coordinates, cut_angle):
    ...
    bond_index = np.asarray(angle_check_index[angle_check.flatten()], dtype=int)
...
def check_angle_water(atoms_in_distance, oxygen_coordinates, hydrogen_coordinates, cut_angle):
    ...
    hydrogen_index = np.zeros(a_index.size, dtype=int)
...
def filter_occupancy(dictionary, min_occupancy):
    for key in dictionary: 
        ...
        break
    if dtype == int: values = np.array(np.array(list(dictionary.values())) < np.inf, dtype=np.float)
...
def string_in_columns(s):
    ...
    rotated_elements = np.array(elements, dtype=str).T
...
def rgb_to_string(rgb):
    r,g,b,a = (np.array(rgb) * 255).astype(int)
...
def biological_centrality_2(graph, normalize=False):
    ...
    check_paths = np.ones((len(all_paths), longest), dtype=int)*-1
